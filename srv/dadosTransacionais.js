...
module.exports = class DadosTransacionais extends cds.ApplicationService { init() {

  const { PedidoHeader, PedidoItem } = cds.entities('DadosTransacionais')

  this.before (['CREATE', 'UPDATE'], PedidoHeader, async (req) => {
    console.log('Before CREATE/UPDATE PedidoHeader', req.data)
  })
  this.after ('READ', PedidoHeader, async (pedidoHeader, req) => {
    console.log('After READ PedidoHeader', pedidoHeader)
  })
  this.before (['CREATE', 'UPDATE'], PedidoItem, async (req) => {
    console.log('Before CREATE/UPDATE PedidoItem', req.data)
  })
  this.after ('READ', PedidoItem, async (pedidoItem, req) => {
    console.log('After READ PedidoItem', pedidoItem)
  })
